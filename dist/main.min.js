...
<<<<<<< HEAD
  var teamAccoJS = function teamAccoJS() {
    var oTeamlink = document.querySelectorAll('.team__headline');
    oTeamlink.forEach(function (personName) {
      personName.addEventListener("click", function (e) {
        e.preventDefault();
        var activePerson = document.querySelector(".team__item.js-team__item");

        if (activePerson) {
          var teamAccordeonDetails = activePerson.querySelector(".team__text");
        ...
        if (!activePerson || activePerson.querySelector(".team__headline") != e.target) {
          var currentPerson = e.target.closest(".team__item");
          currentPerson.classList.add("js-team__item");
          var currentPersonInfo = currentPerson.querySelector(".team__text");
...
(function () {
  var verticalAcco = function verticalAcco() {
    ...
    // }
    var oMenulink = document.querySelectorAll('.carte__title');
    oMenulink.forEach(function (personName) {
      personName.addEventListener("click", function (e) {
        e.preventDefault();
        var activePerson = document.querySelector(".js-carte__item");

        if (activePerson) {
          var teamAccordeonDetails = activePerson.querySelector(".carte__desc");
...
=======
  ...
  for (var i = 0; i < teamItems.length; i++) {
    teamItems[i].addEventListener('click', function () {
      ...
      var teamText = curItem.querySelector('.team__text');
      var textBlock = teamText.firstElementChild;
      var reqHeight = textBlock.getBoundingClientRect().height;

      if (curItem.classList.contains('js-team__item')) {
        //открыт
        ...
        teamText.style.height = "".concat(reqHeight, "px");
      }
    });
  }
})();
...
(function () {
  var carteItems = document.querySelectorAll('.carte__item');
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = carteItems[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var eachItem = _step.value;
      eachItem.addEventListener('click', function (event) {
        var curItem = event.currentTarget;
        var carteDesc = curItem.querySelector('.carte__desc');

        if (curItem.classList.contains('js-carte__item')) {
          //открыт
...
(function () {
  ...
  var logo = document.querySelector('.logo__full-menu');
...
(function () {
  ...
  var placemarks = [{
    ...
    ballonContent: ['<div class="map__balloon">', // '<img class="map__burger-img" src="img/" alt="Burger"/>',
    'Самые вкусные бургеры у нас! Заходите по адресу: ул.Литералов, д. 19', '</div>']
  }, {
    latitude: 59.94,
    longitude: 30.25,
<<<<<<< HEAD
    ...
    ballonContent: ['<div class="map__balloon">', // '<img class="map__burger-img" src="img/" alt="Burger"/>',
    'Самые вкусные бургеры у нас! Заходите по адресу: Малый проспект В.О., д. 64', '</div>']
=======
    hintContent: '<div class="map__hint"> Малый проспект В 0, д. 64 </div>',
    ballonContent: ['<div class="map__balloon">', // '<img class="map__burger-img" src="img/" alt="Burger"/>',
    'Самые вкусные бургеры у нас! Заходите по адресу: Малый проспект В 0, д. 64', '</div>']
>>>>>>> master
  }, {
    latitude: 59.93,
    longitude: 30.34,
    ...
    ballonContent: ['<div class="map__balloon">', // '<img class="map__burger-img" src="img/" alt="Burger"/>',
    'Самые вкусные бургеры у нас! Заходите по адресу: наб. реки Фонтанки, д. 56', '</div>']
  }, {
    latitude: 59.91,
    longitude: 30.27,
    hintContent: '<div class="map__hint"> наб. Обводного канала, д. 223-225СК </div>',
    ballonContent: ['<div class="map__balloon">', // '<img class="map__burger-img" src="img/" alt="Burger"/>',
    'Самые вкусные бургеры у нас! Заходите по адресу: наб. Обводного канала, д. 223-225СК', '</div>']
  }],
      geoObjects = [];
...
(function () {
  ...
  function createModal() {
    ...
    var contentBlock = container.querySelector('.popup__content');
  ...
  var clearBtn = myForm.querySelector('#reset');
...
(function () {
  var OnePageScroll = function OnePageScroll() {
    ...
    var performTransition = function performTransition(sectionEq) {
      if (!inscroll) {
        ...
        visible.css({
          ...
          "-webkit-transform": "translateY(".concat(position, ")")
        });
      }

      setTimeout(function () {
        inscroll = false;
        $(".fixed-menu_item").eq(sectionEq).addClass("fixed-menu_item_active").siblings().removeClass("fixed-menu_item_active");
      }, 1000 + 300);
    };

    var defineSections = function defineSections(sectionsList) {
      var activeSection = sectionsList.filter(".is-active");
      return {
    ...
    if (isMobile) {
      $(window).swipe({
        swipe: function swipe(event, direction, distance, duration, fingerCount, fingerData) {
          var scrollDirection = direction === 'down' ? 'down' : 'up';
...
(function () {
  ...
  var templateR = document.querySelector("#modal-reviews").innerHTML;
  var modalR = createModal();
  var wrapper = document.querySelector(".wrapper");

  for (var i = 0; i < reviewsBtn.length; i++) {
    reviewsBtn[i].addEventListener('click', function (e) {
      event.preventDefault();
      var paragraph = e.currentTarget.parentNode.childNodes[3].innerHTML;
      var headline = e.currentTarget.parentNode.firstElementChild.innerHTML;
      modalR.setContent(headline, paragraph);
      modalR.open(); //   setTimeout(e=>{
      //     modalR.close();
      // }, 6000);
    });
  }
  ...
  function createModal() {
    ...
    var contentBlock = container.querySelector('.popup__content-text');
    var headlineBlock = container.querySelector('.popup__content-headline');
    ...
    return {
      ...
      setContent: function setContent(name, content) {
        contentBlock.innerHTML = content;
...
(function () {
  ...
  var slidesInView = 1;
  var step = items.firstElementChild.getBoundingClientRect().width;
  var maxRight = (items.children.length - slidesInView) * step;
  var maxLeft = items.children.length * step;
  var minRight = 0;
  var currentRight = 0;
  arrowRight.addEventListener('click', function (event) {
    ...
    if (currentRight < maxRight) {
      currentRight += step;
      item.style.right = "".concat(currentRight, "px");
    } else {
      currentRight = 0;
      items.style.right = 0;
    }
  ...
  $('.consist__link').on('touchstart', function (e) {
    $('.consist.list').css('opacity', '1');
  });
  $('.consist__link').on('touchend', function (e) {
...
var soundControl;
var intervalId;
var soundLevel; // документ полностью загружен

$().ready(function () {
  video = document.getElementById("player"); // вешаем обработчик события onclick на тег video
  ...
  durationControl.addEventListener('mousedown', stopInterval); // durationControl.addEventListener('click',setVideoDuration);

  durationControl.addEventListener('mouseup', setVideoDuration);
  durationControl.min = 0;
  durationControl.value = 0; // обработчики событий для ползунка громокости

  soundControl = document.getElementById("micLevel"); // soundControl.addEventListener('click', changeSoundVolume);

  soundControl.addEventListener('mouseup', changeSoundVolume); // задаем максимальные и минимальные значения громокости

  soundControl.min = 0;
  soundControl.max = 10; // присваиваем ползунку максимальное значение

  soundControl.value = soundControl.max; // обработчик событий для кнопки динамик

  var micControl = document.getElementById("mic");
  var soundMic = document.querySelector('.sound__mic');
...
function playStop() {
  ...
  durationControl.max = video.duration; // проверим стоит ли видео на паузе, если да то продолжим воспроизведение. Если, наоборот, проигрыавыется, то остановим.
...
function updateDuration() {
  durationControl.value = video.currentTime; // console.log(video.currentTime)
}
/*
    Управление звуком
*/
